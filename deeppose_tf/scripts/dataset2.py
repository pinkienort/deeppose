...
class PoseDataset2(dataset_mixin.DatasetMixin):

    """Original PoseDataset loads images data in first time.
    So cpu usage to get batch is low, but memory usage is high.
    PoseDataset2 is different to PoseDataset, when get_example()
    is callled, a image is loaded from the file to reduce total
    memory amount in running. Of course, cpu utilization is higher.
    ...
    def __init__(self, csv_fn, img_dir, im_size, fliplr=False,
        ...
        self.base = self.checked_base()
        logging.info('{} is ready'.format(csv_fn))

    def checked_base(self):
        """Check images. In instantiation of this class,
        pick invalid line out to prevent raising error in running."""
        base = []
        for i, line in tqdm(enumerate(csv.reader(open(self.csv_fn)))):
            if self.load_image(line) is not None:
                base.append(line)
        return base
    ...
    def load_image(self, line):
        """One iteration function of the original load_images()"""

        image_fname = line[self.fname_index]
        image_path = os.path.join(self.img_dir, image_fname)

        ## load image
        image = cv.imread(image_path) # HWC BGR image
        flag_downscale = self.should_downscale_images and image.shape[0] > self.downscale_height # boolean

        ## resize image (downscaling)
        if flag_downscale:
            downscale_factor = float(image.shape[0]) / self.downscale_height
            fx = fy = 1.0/ downscale_factor
            image = cv.resize(image, None, fx, fy)
        ...
        if flag_downscale:
            joints /= downscale_factor

        # clip joints to fit the image
        should_skip_joints = False
        for i_joint in range(joints.shape[0]):
            if np.all(is_valid_joints[i_joint]):
                if joints[i_joint][0] - image.shape[1] > 3 or joints[i_joint][1] - image.shape[0] > 3:
                    print('Skipping joint with incorrect joints coordinates. They are out of the image.\n'
                            'image: {}, joint: {}, im.shape: {}'.format(image_path, joints[i_joint], image.shape[:2]))
                    return None
                else:
                    joints[i_joint][0] = np.clip(joints[i_joint][0], 0, image.shape[1])
                    joints[i_joint][1] = np.clip(joints[i_joint][1], 0, image.shape[0])
        ...
        # Ignore small label regions smaller than min_dim
        if bbox[2] < self.min_dim or bbox[3] < self.min_dim:
            print('Skip small person. image: {}'. format(image_path))
            return None
        ...
        return image_path, np.array(image), np.array(joints), \
                np.array(is_valid_joints, dtype=np.bool), np.array(bbox)
    ...
    def get_example(self, i, gcn=None, bbox_extension_range=None, shift=None):
        ...
        if shift is None:
            shift = self.shift
        image_path, image, joints, is_valid_joints, bbox = self.load_image(self.base[i])
        ...
        if self.should_return_bbox:
            ...
            return image, joints, is_valid_joints, crop_bbox, bbox
        return image, joints, is_valid_joints
