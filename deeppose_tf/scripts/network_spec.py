...
def training_convnet(net, loss_op, fc_lr, conv_lr, optimizer_type='adagrad',
    ...
    with net.graph.as_default():
        ...
        with tf.control_dependencies(update_ops):
            conv_tran_op = conv_optimizer.apply_gradients(list(zip(conv_grads, conv_vars)), name='conv_train_op')
        fc_tran_op = fc_optimizer.apply_gradients(list(zip(fc_grads, fc_vars)),
